#!/usr/bin/env python3
"""Render a markdown explainer into a styled, self-contained HTML page.
...
Usage:
    python3 docs/build_html.py                       # flowchart.md -> flowchart.html
    python3 docs/build_html.py IN.md OUT.html --title "..." --subtitle "..." \
...
header.hero .chip a{color:#fff; text-decoration:none; border-bottom:1px solid rgba(255,255,255,.6)}
header.hero .chip a:hover{border-bottom-color:#fff}
nav.toc{
...
pre{
  background:#0f2233; color:#dbe7f2; padding:14px 16px; border-radius:10px;
  overflow-x:auto; font-size:13px; line-height:1.5;
  font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,"Liberation Mono",monospace;
}
pre code{background:transparent; color:inherit; padding:0; font-size:inherit}
main.wrap > h1{font-size:32px; margin:6px 0 4px}
main.wrap > h1 + h3{color:var(--muted); font-weight:500; margin-top:0}
...
DEFAULT_TITLE = "MSETCL — Asset Retirement, Scrap Declaration &amp; Disposal Policy"
DEFAULT_SUBTITLE = ("The complete policy, explained as flow charts. Built from <code>Policy.pdf</code> "
                    "and <code>MSA wardha new scrapping policy.pdf</code> in this repository.")
DEFAULT_CHIPS = [
    "Part 1 — Asset Retirement",
    "Part 2 — Scrap Declaration &amp; Disposal",
    "CLARC → ZSC → CO R&amp;D → Competent Authority → MSTC",
    "8 flow charts + document-trail &amp; deadline tables",
]
...
def main() -> None:
    ...
    ap.add_argument("input", nargs="?", default=str(MD))
    ap.add_argument("output", nargs="?", default=str(OUT))
    ap.add_argument("--title", default=DEFAULT_TITLE)
    ap.add_argument("--subtitle", default=DEFAULT_SUBTITLE)
    ap.add_argument("--chips", default=None,
                    help="pipe-separated list of chips (HTML allowed)")
    args = ap.parse_args()
    ...
    body = convert(md)
    toc = build_toc(md)
    chips = DEFAULT_CHIPS if args.chips is None else args.chips.split("|")
...
<footer>
  Generated by <code>docs/build_html.py</code> from <code>{src.name}</code>.
